...
- **Power Automate「條件 Condition」卡沒有「進階模式」**（新版 designer；「進階模式」只有「篩選陣列」有）：要塞複雜布林運算式 → 放**左值(fx)**、運算子選**等於**、右值**用 fx 打 `true`**（boolean）。右值直接在格子打字 `true` 會變字串 `'true'`，`equals(布林true,'true')` 永遠 false → 條件永遠不成立、永遠不觸發 True 分支。
