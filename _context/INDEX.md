...
- **2026-08-20 新發現（TaskLog B15）**：1000／1010 的行事曆視窗擋不掉「隔日 UTC 午夜起算」的全天事件，08-21 的假被寫進 08-20 的 `field_8`（塗文鳳、林寬祐）。修法＝在 `FilterEvents` 加事件日期閘（`起日 ≤ 今日 < 迄日`），兩支修正包**已匯入並實跑通過**（08-20 使用者驗證）。待觀察：08-21 當天該寫的是否會正確寫入。
- **2026-08-27 新發現（TaskLog B16）**：個人 App（ScrToday）**沒有任何 `Exit()`／離開入口**，同事在手機上送出後只能用系統的 App 列表強制關掉。已加 `btnExit`（`Exit()`）到 `ScrToday_paste.pa.yaml`，貼上指南 `workingfiles/canvas/ScrToday_加離開鈕_2026-08-27.html`，**待貼上 Studio 並手機實測**。ScrAdmin 同病未動，待 PO 決定。
